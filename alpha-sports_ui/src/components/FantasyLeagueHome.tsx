...
const FantasyLeagueHome = () => {
  ...
  useEffect(() => {
    ...
    setActiveTab(1);
    setFixtures(responseData["current"]);
    getFantasyPlayers();
  }, []);
  ...
  const handleTabChange = (
    ...
    if (data.activeIndex == 0) {
      ...
      setFixtures(responseData["previous"]);
    } else {
      setActiveTab(1);
      setShowPrevious(false);
      setFixtures(responseData["current"]);
    ...
    <div style={{ height: "478px", overflow: "scroll" }}>
      <Table inverted>
        <Table.Header>
          <Table.Row>
            <Table.HeaderCell>
              <h4 style={{ marginLeft: "30%" }}>Home Team</h4>
            </Table.HeaderCell>
            {showPrevious && <Table.HeaderCell></Table.HeaderCell>}
            <Table.HeaderCell>
              <h4 style={{ marginLeft: "30%" }}>Away Team</h4>
            </Table.HeaderCell>
          </Table.Row>
        </Table.Header>
  ...
  const panes = [
    {
      ...
      render: () => <Tab.Pane>{tableContent}</Tab.Pane>,
    },
    {
